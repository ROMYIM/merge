...
public class PlatinumController extends BaseLogin{
    ...
    public String baseLogin(int id, String code, String sn, String mac, MongoTemplate mongo) {       
        String flag = "fail", result = "", type = "platinum";
        AgreementAccountBean account = new AgreementAccountBean();
        mongoTemplate = mongo;
        try {
            String url = "http://majood.net/iptv/API-V4.php?mode=active&code="+code+"&mac="+mac+"&sn="+sn+"&raw=yes";
            result = HttpClientUtil.sendHttpGet(url);
            //容易请求超时!!!!!播放链接会变，因为密码会变
            ...
            if (json.getString("status").equals("100")) {
                ...
                url = "http://majood.net/iptv/API-V4.php?mode=packages&code="+code+"&mac="+mac+"&sn="+sn+"&raw=yes";
                result = HttpClientUtil.sendHttpGet(url);
                ...
                if (categoryArr.size() > 0) {
                    for (int i=0; i<categoryArr.size(); i++) {
                        ...
                        getChannel(category.getString("channels"), category.getString("category_name"), user_agent, code, mac, sn, "0");
                    }
                }
                //movie
                url = "http://majood.net/iptv/API-V4.php?mode=movies_cat&code="+code+"&mac="+mac+"&sn="+sn+"&raw=yes";
                result = HttpClientUtil.sendHttpGet(url);
                    ...
                    for (int m = 0; m < movieJson.size(); m++) {
                        ...
                        if (subcatArr.size() >0) {
                            for (int n = 0; n < subcatArr.size(); n++) {
                                ...
                                url = "http://majood.net/iptv/API-V4.php?mode=movies_list&code="+code+"&mac="+mac+"&sn="+sn+"&catid="+subcat.getString("sub_id")+"&raw=yes";
                                result = HttpClientUtil.sendHttpGet(url);
                                getChannel(result, subcat.getString("sub_name"), user_agent, code, mac, sn, subcat.getString("sub_id"));
                            }
                        }
                    }
                }
    ...
    public void getChannel(String channels, String categoryname, String userAgent, 
            String code, String mac, String sn, String categoryid)  {
        try {           
            JSONArray json = JSONArray.parseArray(channels); 
            List<PlayUrlBean> list = new LinkedList<PlayUrlBean>();
            String stream_id = "", category_id = "", stream_name = "", stream_icon = "", stream_url = "", type = "", iptv = "platinum";
            if (json.size() > 0) {
