...
(function ($) {
	...
	if (document.location.href.indexOf('http://') != 0)	{
		var path = '/static/js/treetable/script/treeTable/';  
	} else {
		var path = SITE_URL + '/static/js/treetable/script/treeTable/';
	}
        ...
        $treeTable.click(function (event) {
            ...
            if ($target.attr('arrow')) {
                ...
                if (className == css['AN'] + ' ' + css['HLO'] || className == css['AN'] + ' ' + css['HO']) {
                    ...
                    //alert("test2"+id);
                    //setSelectImput(id);
                    if(opts.arrayOperate){
                    	opts.arrayOperate(0,initParameter.open,id);
                	}
                    //关闭所有孩子的tr
                    shut(id);
					return;
